...
The **Agent playbook** saves research focus, narrative detail, preferred evidence age, and a target of **3–10 independent publishers**. Research Desk and Sleepers also offer a per-run **Source target** override; leave it on **Use Agent playbook** to inherit the saved default. The target applies across the entire report, not per player or position, and includes both known and newly discovered publishers. Named-source refreshes remain limited to the named publisher. These preferences are snapshotted when each job queues (including scheduled jobs); changing the playbook does not rewrite queued or historical assignments. Explicit scheduled overrides stay fixed; schedules using the playbook inherit its current setting when they queue. Source count and freshness are evidence targets, not guarantees. More sources can take more time and subscription usage; more than five ranking boards requires desktop 0.1.11 or newer.
